...
// Versione estesa: aggiunta sistema fornitori
// Su questo progetto abbiamo collaborato usando Live Share e GitHub
// Progetto Sartoria (con fornitori)
...
#define FILEFORNITORI "Fornitori.txt"
...
FILE *Preset;
// File usato per salvare i fornitori
FILE *FFornitori;
...
struct scheda{
	...
	char fornitore[MAXSTRING];        // Fornitore (nome)
...
// Nuova struttura: Fornitori
struct fornitore{
	char nome[MAXSTRING];
	char provenienza[MAXSTRING];
	char specialita[100]; // es: "lana" oppure "lana,cotone"
};
struct fornitore fornitori[MAXFORNITORI];
int FornCount = 0;
...
int nuovoFornitore();
int mostraFornitori();
int eliminaFornitore(char nome[]);
...
int main(){
	...
	caricaFornitori(); // carico i fornitori all'avvio
	system(CLEAR);
	...
	do{
		...
		switch(scelta){
			...
			case 34:
				...
				salvaFornitori(); // salvo i fornitori
				co(2);
			...
			case 51:
				if(nuovoFornitore()==0){
					co(2);
					printf("Fornitore aggiunto con successo!\n");
					co(7);
				}else{
					errore("Impossibile aggiungere fornitore: limite raggiunto!\n");
				}
				pausa("\nContinua...\n");
				break;
			case 52:
				if(mostraFornitori()==1){
					errore("Non ci sono fornitori al momento!\n");
				}
				pausa("\nContinua...\n");
				break;
			case 53:
				printf("Inserisci il nome del fornitore da eliminare: ");
				scanf(" %s",filter);
				if(eliminaFornitore(filter)==0){
					co(2);
...
// --- Funzioni fornitori ---
// Carica i fornitori da FILEFORNITORI
void caricaFornitori(){
	int i=0;
	FFornitori = fopen(FILEFORNITORI, "r");
	if(FFornitori==NULL){
		// file non presente: nessun fornitore
		FornCount = 0;
		return;
	}
	fscanf(FFornitori, "%d", &FornCount);
	if(FornCount>MAXFORNITORI) FornCount=MAXFORNITORI;
	for(i=0;i<FornCount;i++){
		fscanf(FFornitori, "%s %s %s", fornitori[i].nome, fornitori[i].provenienza, fornitori[i].specialita);
	}
	fclose(FFornitori);
}

// Salva i fornitori su FILEFORNITORI
void salvaFornitori(){
	int i;
	FFornitori = fopen(FILEFORNITORI, "w");
	if(FFornitori==NULL) return;
	fprintf(FFornitori, "%d\n", FornCount);
	for(i=0;i<FornCount;i++){
		fprintf(FFornitori, "%s %s %s\n", fornitori[i].nome, fornitori[i].provenienza, fornitori[i].specialita);
	}
	fclose(FFornitori);
}

// Aggiunge un nuovo fornitore (usa input standard)
int nuovoFornitore(){
	if(FornCount>=MAXFORNITORI) return 1;
	printf("Nuovo fornitore:\n");
	printf("\tNome: "); scanf(" %s", fornitori[FornCount].nome);
	printf("\tProvenienza: "); scanf(" %s", fornitori[FornCount].provenienza);
	printf("\tSpecialita (es: lana,cotone): "); scanf(" %s", fornitori[FornCount].specialita);
	FornCount++;
	salvaFornitori();
	return 0;
}

// Mostra i fornitori a schermo
int mostraFornitori(){
	int i;
	if(FornCount==0) return 1;
	printf("Elenco fornitori:\n");
	for(i=0;i<FornCount;i++){
		printf("%d) %s | %s | %s\n", i+1, fornitori[i].nome, fornitori[i].provenienza, fornitori[i].specialita);
	}
	pausa("[INVIO] Continua...\n");
	return 0;
}

// Elimina un fornitore per nome
int eliminaFornitore(char nome[]){
	int i,j;
	for(i=0;i<FornCount;i++){
		if(strcmp(fornitori[i].nome,nome)==0){
			for(j=i;j<FornCount-1;j++) fornitori[j]=fornitori[j+1];
			FornCount--;
			salvaFornitori();
			return 0;
		}
